...
if (isset( $_GET['id'] ) && isset( $_POST)  && !empty( $_POST )) {
    $id = (integer) $_GET['id'];
    $sql = 'UPDATE posts SET title="'.$_POST['title'].'", content="'.$_POST['content']. '", description="'.$_POST['description'].'",category_name="'.$_POST['category']. '" WHERE id='.$id.';';
...
if (isset( $_GET['id'])) {
    ...
    if ( $result = $connection->query( $sql ) ) {
        ...
        $category = $_POST['category'] ?? $post['category_name'];
    } else {
...
$sql= 'SELECT * FROM posts(title, content, description, category_name ) VALUES("'.$title.'", "'.$content.'", "'.$description.'", "'.$category.'");';
...
<html>
    ...
    <body>
        ...
        <form action="#" method="POST">
            ...
            <label for="category"> 
                Category:
                <input type="text" placeholder= "Enter a category..." name= "category" <?php if (isset( $category)) echo ' value="' . $category.'"'; ?>>
            </label>
