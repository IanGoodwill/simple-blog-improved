        ...
        if(mysqli_num_rows($query) > 0){
           ...
           echo "<li>";
            ...
            if($current_page_num > 1){
               echo '<a href="?id='.$prev_page.'" class="page_link">Prev</a>';
            }
            // show all number of pages
            for($i = 1; $i <= $total_page; $i++){
                //highlight the current page number
                if($i == $current_page_num){
                    echo '<a href="?id='.$i.'" class="page_link active_page">'.$i.'</a>';
                }else{
                    echo '<a href="?id='.$i.'" class="page_link">'.$i.'</a>';
                }
                
            }
            // showing next button and check this is last page
            if($total_page+1 != $next_page){
               echo '<a href="?id='.$next_page.'" class="page_link">Next</a>';
            }
